...
def evalutate_cocktail_performance(
    ...
    if not full_predictions_df.empty:
        for (iteration, run_label), group in full_predictions_df.groupby(['iteration', 'modeling_method']):
            # if run_label != 'bootstrap_inverse_frequency_hierarchical_SPLIT':
            #     continue
            ...
            for n in [1, 3, 5]:
                ...
                if run_label == 'inverse_frequency_hierarchical_SPLIT':   
                    full_feature_table_path = os.path.join(bootstrap_dir, iteration, 'merged', 'full_feature_table.csv')
                    if not os.path.exists(full_feature_table_path):
                        print('No full feature table found for:', iteration, ' - ', run_label)
                        continue
                    full_feature_table_df = pd.read_csv(full_feature_table_path)
                    # print('Running cocktail design analysis for:', iteration)
                    cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(group, full_feature_table_df, clustering=False, clustering_method='hierarchical', display_intermediate=False, save_plot=False, top_n=n, generate_plot=False, plot_umap=False, default_cocktails=True)
                    cocktail_design_summary['clustering'] = 'None'
                    cocktail_design_summary['top_n'] = n
                    cocktail_design_summary['run_label'] = 'Default cocktail'
                    bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])

                    cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(group, full_feature_table_df, clustering=True, clustering_method='hierarchical', display_intermediate=False, save_plot=False, top_n=n, generate_plot=False, plot_umap=False, default_cocktails=True)
                    cocktail_design_summary['clustering'] = 'hierarchical'
                    cocktail_design_summary['top_n'] = n
                    cocktail_design_summary['run_label'] = 'Default cocktail'
                    bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])

                    cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(group, full_feature_table_df, clustering=True, clustering_method='HDBSCAN', display_intermediate=False, save_plot=False, top_n=n, generate_plot=False, plot_umap=False, split_by_strain=False, default_cocktails=True)
                    cocktail_design_summary['clustering'] = 'HDBSCAN'
                    cocktail_design_summary['top_n'] = n
                    cocktail_design_summary['run_label'] = 'Default cocktail'
                    bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])
