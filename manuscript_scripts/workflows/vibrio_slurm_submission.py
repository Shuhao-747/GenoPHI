...
def main():
    ...
    # Analysis options
    use_shap = False                    # Calculate SHAP values (increases runtime)
    annotation_table_path = None        # Path to annotation table (None = not used)
    protein_id_col = "protein_ID"       # Protein ID column name
    
    # Resource parameters
    threads = "20"                       # Number of threads per job (will be overridden by SLURM allocation)
    max_ram = "60"                       # Maximum RAM usage in GB
    time_limit = "24:00:00"             # Time limit for each job (set in SLURM)
    ...
    cmd = [
        ...
        "--threads", threads,
        "--max_ram", max_ram,
        "--time_limit", time_limit,  # Set a time limit for the job
    ]
